...
ebot (Expedient Bot) is an open-source platform for implementing Model Context Protocol (MCP) technologies. It provides MCP hosting (Docker/Kubernetes), an MCP registry, an MCP gateway, and ebot Chat (a built-in chat client supporting OpenAI and Anthropic models).

This is a customized fork of the Obot platform, tailored for Expedient's enterprise requirements.
...
### Building
```bash
make build            # Build Go binary to bin/ebot
...
- `/pkg` - Core Go packages
  ...
  - `storage/` - CRD-style storage layer with resource types in `apis/ebot.expedient.cloud/v1/`
...
**Server Types:**
- **Single-user**: No multitenancy - ebot deploys an instance per user. Stored as `MCPServerCatalogEntry` with runtime `npx`, `uvx`, or `containerized`
- **Multi-user**: Supports multitenancy - one instance for all users. Stored as `MCPServer`
- **Remote**: Runs outside ebot. Stored as `MCPServerCatalogEntry` with runtime `remote`
...
ebot serves the MCP Registry API (open standard) at `/v0.1` routes.
...
- `github.com/chad-atexpedient/ebot` (main)
